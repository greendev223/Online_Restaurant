...
class Addons_model extends CI_Model {
	...
	{
		$this->db->where('add_store_id',$id)
			->delete($this->table);

		if ($this->db->affected_rows()) {
			$this->db->where('add_store_id',$id)->delete($this->table2);
			return true;
		} else {
			return false;
		}
  ...
  public function menuaddons_delete($id = null)
	...
	{
		return $this->db->where('add_store_id',$data["add_store_id"])
    ...
    public function read_addons($limit = null, $start = null)
	{
	   $this->db->select('*');
        ...
        $this->db->order_by('add_store_id', 'desc');
	...
	{
	    $this->db->select('menu_add_on.*,item_foods.ProductName,add_store.add_store_name');
...
public function count_menuaddons()
	{
        ...
        $this->db->from($this->table2);
		$this->db->join('item_foods','menu_add_on.menu_id = item_foods.ProductsID','left');
		$this->db->join('add_store','menu_add_on.add_store_id = add_store.add_store_id','inner');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->num_rows();  
        }
